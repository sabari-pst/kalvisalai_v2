...
import AddAttendancePercentage from "./addAttendancePercentage";
import { listAttendancePercentages } from "../../../../models/academicYears";
import EditAttendncePercentage from "./editAttendncePercentage";

const AttendancePercentageSettings = (props) => {
  ...
  const getReport = () => {
    ...
    listAttendancePercentages().then((res) => {
      if (res) {
        setDataList(res);
        setDataView(res);
      }
  ...
  const handleDeleteClick = (item) => {
    ...
    axios
      .post(ServiceUrl.UTILITIES.DELETE_ATTENDANCE_PERCENTAGE, form)
      .then((res) => {
  ...
  return (
    <>
      <CardFixedTop title="Attendance Percentage Settings">
                      ...
                      <tbody>
                        {dataView.map((item, i) => {
                          return (
                            ...
                            >
                              <td>{i + 1}</td>
                              <td align="center">{item.percentage_from}</td>
                              <td align="center">{item.percentage_to}</td>
                              <td>{item.name}</td>
                              <td align="center">
                                <ModuleAccess
      ...
      <PsModalWindow
        title="Add Attendance Percentage"
        onHide={(e) => setShowAdd(false)}
        show={showAdd}
        size="sm"
      >
        <AddAttendancePercentage onSuccess={(e) => getReport()} />
      </PsModalWindow>
...
export default withRouter(AttendancePercentageSettings);
