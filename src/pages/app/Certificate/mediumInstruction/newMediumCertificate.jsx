...
const NewMediumCertificate = (props) => {
  ...
  const loadTemplate = () => {
    setLoader(true);
    getStudentTemplate(CERTIFICATE_TEMPLATE_NAMES.STUDENT_MEDIUM).then(
      (res) => {
  ...
  const getContent = () => {
    ...
    ar["{{studing_medium}}"] = upperCase(
      studentField("medium_of_instruction") == "" ||
        studentField("medium_of_instruction") === null
        ? "_________"
        : studentField("medium_of_instruction")
    );
  ...
  const handleSaveClick = () => {
    ...
    axios
      .post(ServiceUrl.STUDENTS.SAVE_MEDIUM_CERTIFICATE, form)
      .then((res) => {
...
export default NewMediumCertificate;
