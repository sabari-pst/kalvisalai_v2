...
const StudentBirthDayList = (props) => {
    ...
    const loadData=()=>{
        ...
        axios.post(ServiceUrl.DASHBOARD.STUDENTS_BIRTHDAY_LIST, form).then(res=>{
    ...
    return (
        <>
            ...
            <Card>
                <Card.Header className="fw-bold">Students Birthday List <Badge>{dataList.length}</Badge>
                                ...
                                <tbody>
                                    {dataView.map((item, i)=>{
                                        return <tr key={i}>
                                            ...
                                            <td>{item.mobile}</td>
                                            <td>{calculateAge(item.dob)} /  {upperCase(item.gender)=='MALE' ? 'M' : 'F'}</td>
                                            <td>{item.degree_name}-{item.course_name} ({upperCase(item.course_type)=='SELF' ? 'SF' : 'R'})</td>
                                            <td>{yearBySem(item.semester)}</td>
                                        </tr>
                                    })}
                                </tbody>
                            </table>
...
export default withRouter(StudentBirthDayList);
