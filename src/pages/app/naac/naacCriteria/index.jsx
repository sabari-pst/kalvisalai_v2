...
import EditNaacCriteria from "./editNaacCriteria";
import AddNaacCriteria from "./addNaacCriteria";
import ModuleAccess from "../../../../context/moduleAccess";
import { listCriteria } from "../../../../models/naac";

const NaacCriteria = (props) => {
  ...
  const getReport = () => {
    ...
    listCriteria("1", "&" + jsonToQuery(getRandomuserParams(tableParams))).then(
      (res) => {
        if (res) {
  ...
  const handleDeleteClick = (item) => {
    ...
    axios.post(ServiceUrl.NAAC.REMOVE_CRITERIA, form).then((res) => {
  ...
  const columns = [
    ...
    { title: "Criteria Title", dataIndex: "criteria_title" },
    {
      title: "Manage",
      dataIndex: "id",
      width: 100,
      ...
      render: (text, record, index) => (
        <>
          <ModuleAccess module="naac_criteria_titles" action="action_update">
            <Button
              size="sm"
              variant="transparent"
              title="Edit Group"
          ...
          <ModuleAccess module="naac_criteria_titles" action="action_delete">
            <Button
              size="sm"
              variant="transparent"
              title="Remove Group"
  ...
  return (
    <>
      <CardFixedTop title="NAAC Criteria">
        <ul className="list-inline mb-0">
          <ModuleAccess module="naac_criteria_titles" action="action_create">
          ...
          <Row className="mt-3">
            ...
            <Col md={8}>
              <div className="text-end fs-sm fw-bold">
                Total no of Criteria : {tableParams.pagination.total || 0}
              </div>
            </Col>
...
export default withRouter(NaacCriteria);
