...
const WebEvents = (props) => {
  ...
  const columns = [
    ...
    {
      title: "Event Date",
      dataIndex: "active_from_date",
      width: 160,
      align: "center",
      render: (text, record, index) =>
        momentDate(record.active_from_date, "DD/MM/YYYY"),
    },
    {
      title: "Category",
      dataIndex: "category",
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <>
          <ModuleAccess module="cms_web_events" action="action_update">
            <Link
              to={`/app/web/events/edit/${record.id}/${record.seo_slug}`}
              className="btn btn-transparent"
            >
              <i className="fa-solid fa-pen fs-6"></i>
            </Link>
          </ModuleAccess>
          <ModuleAccess module="cms_web_events" action="action_delete">
            <Button
              size="sm"
              variant="transparent"
              title="Remove Page"
  ...
  return (
    <>
      <CardFixedTop title="Events">
        <ul className="list-inline mb-0">
          <ModuleAccess module="cms_pages" action="action_create">
            <li className="list-inline-item">
              <Link
                to="/app/web/events/new"
                className="btn btn-white border-start"
              >
                <i className="fa-solid fa-plus  fs-5 px-1"></i> Add Event
              </Link>
            </li>
          </ModuleAccess>
          <li className="list-inline-item">
        ...
        <Row className="mt-3">
          ...
          <Col md={8}>
            <div className="text-end fs-sm fw-bold">
              {tableParams.pagination.total || 0} Events
            </div>
          </Col>
        </Row>
        <Card className="mt-3">
...
export default withRouter(WebEvents);
