...
import NewBonafiedCertificate from "./newBonafiedCertificate"; 

const StudentBoanfiedCertificates = (props) => {
  ...
  const getReport = () => {
    ...
    axios
      .get(
        ServiceUrl.STUDENTS.LIST_BONAFIED_CERTIFICATES +
          "?&" +
  ...
  const handleDeleteClick = (item) => {
    ...
    axios
      .post(ServiceUrl.STUDENTS.DELETE_BONAFIED_CERTIFICATE, form)
      .then((res) => {
        if (res["data"].status == "1") {
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "left",
      render: (text, record, index) =>
        momentDate(record.generated_on, "DD-MM-YYYY hh:mm"),
    },
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <>
          <ModuleAccess module="students_cert_bonafied" action="action_print">
            <Button
              size="sm"
              variant="transparent"
              title="Print"
          ...
          <ModuleAccess module="students_cert_bonafied" action="action_delete">
            <Button
              size="sm"
              variant="transparent"
              title="Remove "
  ...
  return (
    <>
      <CardFixedTop title="Bonafied Certificates">
        <ul className="list-inline mb-0">
          <ModuleAccess module="students_cert_bonafied" action="action_create">
      ...
      <PsOffCanvasWindow
        show={showAdd}
        title="New Bonafied Certificate"
        onHide={(e) => {
          setShowAdd(false);
          setEditData([]);
        }}
...
export default withRouter(StudentBoanfiedCertificates);
