...
const EditNaacSsrReport = (props) => {
  ...
  const getReport = () => {
    setLoader(true);
    axios
      .get(ServiceUrl.NAAC.GET_REPORT + "?id=" + props.match.params.id)
      .then((res) => {
        if (res["data"].status == "1") {
          setDataSource(res["data"].data);
          setEditorContent(res["data"].data.report_content);
        } else {
  ...
  useEffect(() => {
    let s = ssrGroups.find((item) => item.id == field("criteria_group_id"));
    if (s) setSelectedGroup(s);
    console.log(s);
  ...
  const handleFormSubmit = (e) => {
    ...
    setLoader(true);
    axios
      .post(ServiceUrl.NAAC.UPDATE_REPORT, new FormData(form))
      .then((res) => {
        if (res["data"].status == "1") {
  ...
  const groupTitleChange = (v, l) => {
    let s = ssrGroups.find((item) => item.id == v);
    if (s) setSelectedGroup(s);
  };

  const field = (fieldName) => {
    if (dataSource && dataSource[fieldName]) return dataSource[fieldName];
  };

  const userAllowedToEditMeta = context.allowedAccess(
    "naac_ssr_report_meta",
    "action_update"
  );

  return (
    <>
      <CardFixedTop title="Update SSR Report">
        <ul className="list-inline mb-0">
          ...
          <li className="list-inline-item">
            ...
            >
              <i className="fa fa-check fs-6 px-1"></i> Update Report
            </Button>
          </li>
        </ul>
      </CardFixedTop>
                ...
                <Row className="">
                  <Col md={2}>
                    ...
                    <Form.Control
                      ...
                      size="sm"
                      readOnly={!userAllowedToEditMeta}
                      required
                    >
                      ...
                      <option
                        value="qnm"
                        selected={
                          field("report_type") == "qnm" ? "selected" : ""
                        }
                      >
                        qnm
                      </option>
                      <option
                        value="qlm"
                        selected={
                          field("report_type") == "qlm" ? "selected" : ""
                        }
                      >
                        qlm
                      </option>
                  ...
                  <Col md={10}>
                    ...
                    <CustomDropDown
                      dataSource={ssrGroups}
                      disabled={!userAllowedToEditMeta}
                      value={selectedGroup.id}
                      options={getOptions()}
                      displayField={(item) =>
                ...
                <Row className="mt-2">
                  <Col md={2}>
                    ...
                    <Form.Control
                      ...
                      size="sm"
                      placeholder="1.1.1"
                      defaultValue={field("report_code")}
                      readOnly={!userAllowedToEditMeta}
                      required
                    />
                  </Col>
                  <Col md={10}>
                    ...
                    <Form.Control
                      ...
                      size="sm"
                      className="fw-bold"
                      defaultValue={field("report_title")}
                      readOnly={!userAllowedToEditMeta}
                      required
                    />
                  </Col>
                ...
                <Row className="mt-2">
                  <Col md={2}>
                    ...
                    <Form.Control
                      ...
                      name="show_in_menu"
                      defaultValue={field("show_in_menu")}
                      required
                    >
...
export default withRouter(EditNaacSsrReport);
