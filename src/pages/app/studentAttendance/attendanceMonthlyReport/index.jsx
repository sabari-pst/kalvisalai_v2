...
const AttendanceMonthlyReport = (props) => {
  ...
  const loadMarkedAttendance = () => {
    ...
    axios
      .post(ServiceUrl.STUDENTS.ATTENDANCE_MONTHLY_REPORT, form)
      .then((res) => {
        if (res["data"].status == "1") {
  ...
  const getAttendanceDayValueForADay = (day, stu, asNumber = false) => {
    ...
    if (s.length < 1) return asNumber ? "0" : "-";
    let lt = upperCase(s?.[0]?.leave_type);
    let pcount = s[0];
    //if (stu.registerno == "221203103") console.log(stu);
    if (asNumber) {
      //if (s[0].absent_hours == stu.att_hour_per_day) return "0";
      let rec = pcount;
      //if (lt == "LA" || lt == "A") return 0;
      //else {
      // calculate total no of presents

      return ParseFloat(parseFloat(rec.morning) + parseFloat(rec.evening), 2);
      //return ParseFloat(pcount / hourCount, 2);
  ...
  const daysPresent = (stu) => {
    ...
    let m = markedStudents.filter((item) => item.student_uuid == stu.uuid);
    m.map((item) => (days = parseFloat(days) + parseFloat(item.present_hours)));

    let coziant = Math.floor(days / hourCount);
    let remindar = days % hourCount;
    days = coziant + "." + remindar;
    /*if (m[0].registerno == "202603102") {
      console.log("Coz : " + coziant);
      console.log("Rem : " + remindar);
