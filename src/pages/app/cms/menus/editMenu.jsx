...
const EditMenu = (props) => {
  ...
  useEffect(() => {
    console.log("ditem", dataItem);
    setSelLinkType(dataItem.linktype);
    addForm.setFieldsValue({
      linktype: dataItem.linktype,
      title: dataItem.title,
      menulink:
        dataItem.linktype === "page" ? dataItem.postid : dataItem.menulink,
      activestatus: parseInt(dataItem.activestatus) === 1 ? true : false,
    });
  }, [dataItem]);
  const LoadPages = () => {
  ...
  const onFinishForm = (values) => {
    ...
    processedValues["id"] = dataItem.id;
    var form = new FormData();
          ...
          {selLinkType === "page" && (
            <>
              ...
              >
                ...
                >
                  {pageData.map((d) => (
                    <Option key={d.id}>{d.seo_slug}</Option>
                  ))}
                </Select>
              </Form.Item>
            </>
